...
// Animate certification boxes on scroll
  const boxes = document.querySelectorAll('.certification-box');

  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        entry.target.classList.add('animate');
        observer.unobserve(entry.target); // remove if you want replay on scroll
      }
    });
  }, {
    threshold: 0.2
